...
echo "Check to see if we've already performed this setup."
if [ ! -e "$SOLR_SETUP_COMPLETE_FILE" ]; then

  echo "Download and expand the Solr module."
  wget -qO- $SOLR_DOWNLOAD | tar xvz -C $SOLR_DOWNLOAD_DIR

  echo "Copy the Solr configuration into place over the default collection1 core."
  sudo cp -a $SOLR_DOWNLOAD_DIR/$SOLR_MODULE_NAME/solr-conf/$SOLR_VERSION/* $SOLR_CORE_PATH/conf/

  echo "Adjust the autoCommit time so index changes are committed in 1s."
  sudo sed -i 's/\(<maxTime>\)\([^<]*\)\(<[^>]*\)/\11000\3/g' $SOLR_CORE_PATH/conf/solrconfig.xml

  echo "Fix file permissions."
  sudo chown -R solr:solr $SOLR_CORE_PATH/conf

  echo "Restart Apache Solr."
  sudo service solr restart

  echo "Create a file to indicate this script has already run."
